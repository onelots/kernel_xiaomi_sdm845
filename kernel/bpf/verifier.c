 ...
 * Note there is a non-zero parent pointer inside each register of the
 * verifier state which realloc_stack_state() copies over. It points to the
 * corresponding register in a previous state which is never reallocated.
 */
				  ...
				  int func_id, struct bpf_call_arg_meta *meta)
{
	...
	/* Error case where ret is in interval [S32_MIN, -1]. */
	...
	if (opcode == BPF_END || opcode == BPF_NEG) {
		...
		if (BPF_SRC(insn->code) == BPF_X) {
			...
			if (BPF_CLASS(insn->code) == BPF_ALU64)
				...
				__mark_reg_known(regs + insn->dst_reg, (u32)imm);
		}
 ...
 * -1 - unknown.
 * Example: "if (reg < 5)" is unknown when register value range is [0,10].
 */
			   ...
			   bool is_jmp32)
{
	...
	if (is_jmp32) {
		...
		if ((reg->umax_value ^ reg->umin_value) & (1ULL << 31)) {
			    ...
			    u8 opcode, bool is_jmp32)
{
	...
	case BPF_JSGT: {
		...
		if (!is_jmp32 || cmp_val_with_extended_s64(sval, false_reg)) {
			false_reg->smax_value = min(false_reg->smax_value,
						 false_smax);
			true_reg->smin_value = max(true_reg->smin_value,
						 true_smin);
		}
		break;
	}
	...
	case BPF_JSLT: {
		...
		if (!is_jmp32 || cmp_val_with_extended_s64(sval, false_reg)) {
			false_reg->smin_value = max(false_reg->smin_value,
						 false_smin);
			true_reg->smax_value = min(true_reg->smax_value,
						 true_smax);
		}
		break;
	}
			     ...
			     struct bpf_insn *insn, int *insn_idx)
{
	...
	if (BPF_SRC(insn->code) == BPF_X) {
		...
		struct bpf_reg_state *src_lo = src_reg;
		...
		if (dst_reg->type == SCALAR_VALUE &&
			...
			if (tnum_is_const(src_lo->var_off))
				reg_set_min_max(
					&other_branch_regs[insn->dst_reg],
					dst_reg, src_lo->var_off.value,
					opcode, is_jmp32);
			else if (tnum_is_const(dst_lo->var_off))
				reg_set_min_max_inv(
					&other_branch_regs[insn->src_reg],
					src_reg, dst_lo->var_off.value,
					opcode, is_jmp32);
			else if (!is_jmp32 &&
		...
		reg_set_min_max(&other_branch_regs[insn->dst_reg], dst_reg,
				insn->imm, opcode, is_jmp32);
	}
...
static int check_ld_abs(struct bpf_verifier_env *env, struct bpf_insn *insn)
{
	 ...
	 */
	mark_reg_unknown(env, regs, BPF_REG_0);
	/* ld_abs loads up to 32-bit skb data. */
					 ...
					 const union bpf_attr *attr)
{
	...
	for (i = 0; i < len; i++) {
		...
		if (!aux[adj_idx].zext_dst) {
			...
			/* The reg argument is only used for BPF_STX, which
			 * insn_no_def() ruled out above.
			 */
			...
			/* A context load can be transformed into a wider load. */
			if (class == BPF_LDX &&
			...
			patch_len = ARRAY_SIZE(rnd_hi32_patch);
		...
		patch = zext_patch;
		patch_len = ARRAY_SIZE(zext_patch);
apply_patch_buffer:
...
patch_call_imm:
		 ...
		 */
		if (!fn->func) {
			verbose(env, "kernel subsystem misconfigured func %d\n",
				insn->imm);
			return -EFAULT;
		}
...
skip_full_check:
	...
	/* Do 32-bit optimization after instruction patching so patched
	 * instructions can be handled correctly.
	 */
